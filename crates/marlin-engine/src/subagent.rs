...
use marlin_config::{AstMode, Config};
use marlin_core::skill::Skill;
use marlin_index::Index;
use marlin_preflight as preflight;
use marlin_providers::{Message, Provider, StreamRequest, ToolCall, ToolCallMsg};
...
/// conversation's).
fn subagent_tools() -> Vec<marlin_providers::ToolDef> {
...
pub fn build_task(skill: &Skill, query: &str) -> Result<String, String> {
    if skill.is_shell() {
        let cmds =
            marlin_skills::executor::resolve_chunks(skill, query).map_err(|e| e.to_string())?;
        let numbered = cmds
            .iter()
            .enumerate()
        ...
        if skill.is_prompt() {
            let prose = marlin_skills::executor::expand_prompt(skill, query).unwrap_or_default();
        ...
        marlin_skills::executor::expand_prompt(skill, query).map_err(|e| e.to_string())
    } else {
        Err(format!(
    ...
    tokio::task::spawn_blocking(move || {
        let search_fn: Option<Box<executor::SearchFn<'_>>> = idx_clone.clone().map(|idx| {
            let f: Box<executor::SearchFn<'_>> = Box::new(move |q: &str, lim: usize| {
                let results = marlin_index::search(&idx, q, lim);
                marlin_index::format_results(&results, q)
            });
            f
        });
        let symbol_search_fn: Option<Box<executor::SymbolSearchFn<'_>>> = idx_clone.map(|idx| {
            let f: Box<executor::SymbolSearchFn<'_>> = Box::new(move |sym: &str, lim: usize| {
                let results = marlin_index::search_symbols(&idx, sym, lim);
                marlin_index::format_symbol_results(&results, sym)
            });
            f
        });
        executor::execute(
...
mod tests {
    use super::*;
    use marlin_core::skill::{Chunk, SkillFormat};
    ...
    use marlin_config::Config;
    use marlin_providers::StreamChunk;
    ...
    async fn subagent_loop_runs_a_tool_call_and_returns_final_summary() {
        ...
        cfg.sandbox_mode = marlin_config::SandboxMode::Permissive; // allow the echo without an approval prompt
