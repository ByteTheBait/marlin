...
pub fn match_skills(input: &str, skills: &[SkillDef]) -> Vec<SkillMatch> {
    ...
    let mut matches: Vec<SkillMatch> = skills.iter().filter_map(|skill| {
        let score = skill.triggers.iter().map(|trigger| {
        ...
        if score > 0 {
            Some(SkillMatch { name: skill.name.clone(), description: skill.description.clone(), score })
        } else {
            None
        }
    }).collect();
