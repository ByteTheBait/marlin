...
// ── Skill types ───────────────────────────────────────────────────────────────
//
// A skill is a name/description/triggers plus an optional prose `body` and a
// list of fenced `{sh}`/`{bash}` `chunks`, run in order:
//
//   chunks | body | behavior
//   -------|------|---------
//   yes    | no   | shell skill — run the chunk(s), return their output
//   no     | yes  | prompt skill — expand the template, feed to the model
//   yes    | yes  | run the chunk(s), then feed prose + output to the model
//
// Chunks don't chain — nothing pipes from one chunk's output into the next.
...
// ── Legacy .toml format (deprecated, parsed for one release) ─────────────────
